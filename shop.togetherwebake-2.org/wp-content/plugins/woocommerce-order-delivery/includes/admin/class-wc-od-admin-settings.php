...
if ( ! class_exists( 'WC_OD_Admin_Settings' ) ) {
	...
	class WC_OD_Admin_Settings {
		...
		public function init_settings_api() {
			...
			if ( 'delivery_day' === $current_section ) {
				include_once 'settings/class-wc-od-settings-delivery-day.php';
				$this->settings_api = new WC_OD_Settings_Delivery_Day( $day_id );
			} elseif ( 'time_frame' === $current_section && isset( $_GET['frame_id'] ) ) { // WPCS: CSRF ok.
				$frame_id = (string) wc_clean( wp_unslash( $_GET['frame_id'] ) );  // WPCS: CSRF ok, sanitization ok.
		...
		public function enqueue_scripts() {
			if ( $this->is_shipping_tab() ) {
				if ( $this->is_calendar_section() ) {
					wp_enqueue_style( 'fullcalendar', WC_OD_URL . 'assets/css/lib/fullcalendar.css' );
					wp_enqueue_style( 'tooltipster', WC_OD_URL . 'assets/css/lib/tooltipster.css' );
				...
				wp_enqueue_style( 'wc-od-settings', WC_OD_URL . 'assets/css/wc-od-settings.css' );
		...
		public function get_settings( $current_section = '' ) {
			...
			if ( 'shipping' === $section ) {
				...
				$settings = array(
					...
					array(
						'id'       => wc_od_maybe_prefix( 'delivery_date_field' ),
						'title'    => __( 'Delivery date field', 'woocommerce-order-delivery' ),
						'desc'     => __( 'Choose the option to determine the behavior of the delivery date field in the checkout form.', 'woocommerce-order-delivery' ),
						'type'     => 'radio',
						'desc_tip' => true,
						'default'  => WC_OD()->settings()->get_default( 'delivery_date_field' ),
						'options'  => array(
							'optional' => __( 'It is an optional field', 'woocommerce-order-delivery' ),
							'auto'     => __( 'It will be the first available date if the customer does not choose one', 'woocommerce-order-delivery' ),
							'required' => __( 'It is a required field', 'woocommerce-order-delivery' ),
						),
					),
