...
abstract class WC_OD_Admin_List_Table {
	...
	public function render_date_filter( $filter ) {
		...
		/**
		 * Filter the additional choices for the date filter.
		 *
		 * @since 1.4.0
		 *
		 ...
		 * @param array $filter  The filter data.
		 */
		$additional_choices = apply_filters( 'wc_od_admin_date_filter_additional_choices', array(
			'empty' => array(
				'value' => 0,
				'label' => $filter['empty'],
			),
		...
		$field_id = 'filter-by-' . esc_attr( $filter['id'] );
		$selected = ( isset( $_GET[ $filter['id'] ] ) ? (int) $_GET[ $filter['id'] ] : 0 );
		?>
		<label for="<?php echo $field_id; ?>" class="screen-reader-text"><?php echo esc_html( $filter['label'] ); ?></label>
		<select name="<?php echo esc_attr( $filter['id'] ); ?>" id="<?php echo $filter['id']; ?>">
			<?php
	...
	public function query_date_filter( $filter ) {
		if ( empty( $_GET[ $filter['id'] ] ) || '0' == $_GET[ $filter['id'] ] ) {
			return false;
		}
	...
	public function get_date_filter_months( $key ) {
		...
		if ( isset( $_GET['post_status'] ) && 'all' !== $_GET['post_status'] ) {
			$extra_checks .= $wpdb->prepare( " AND {$wpdb->posts}.post_status = %s", $_GET['post_status'] );
		} else {
