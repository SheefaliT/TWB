...
if ( ! class_exists( 'WC_OD_Subscription_Admin' ) ) {
	...
	class WC_OD_Subscription_Admin {
		...
		public function subscription_date_updated( $subscription, $date_type ) {
			if ( 'next_payment' !== $date_type || empty( $_POST ) ) {
				return;
			}

			// Check the nonce.
			if ( empty( $_POST['woocommerce_meta_nonce'] ) || ! wp_verify_nonce( $_POST['woocommerce_meta_nonce'], 'woocommerce_save_data' ) ) { // WPCS: sanitization ok.
				return;
			}
			...
			if ( $new_delivery_date && $posted_delivery_date !== $new_delivery_date ) {
				...
				if ( $time_frame_id ) {
					$time_frame = wc_od_get_time_frame( $new_delivery_date, $time_frame_id );
