 ...
 *
 * @since 1.3.0
 * @since 1.5.0 Doesn't return the preferred delivery days.
 *
 * @param mixed $the_subscription Post object or post ID of the subscription.
 * @return array|false An array with the subscription delivery days. False on failure.
 */
function wc_od_get_subscription_delivery_days( $the_subscription ) {
	...
	foreach ( $delivery_days as $index => $delivery_day ) {
		...
		$delivery_days[ $index ]['enabled'] = wc_od_get_delivery_day_status(
			$delivery_day,
			array(
		...
		$delivery_days[ $index ]['time_frames'] = wc_od_get_time_frames_for_delivery_day(
			$delivery_day,
			array(
				'subscription'    => $subscription,
				'shipping_method' => $shipping_method,
...
function wc_od_get_subscription_preferred_delivery_days( $the_subscription ) {
	...
	if ( ! empty( $preferred_days ) ) {
		...
		foreach ( $preferred_days as $index => $preferred_day ) {
			...
			if ( wc_od_string_to_bool( $delivery_days[ $index ]['enabled'] ) ) {
				$delivery_days[ $index ]['enabled'] = wc_od_bool_to_string( $preferred_day['enabled'] );
			}
			...
			if ( ! empty( $preferred_day['time_frame'] ) ) {
				...
				if ( false !== $time_frame_id ) {
					$delivery_days[ $index ]['time_frames'] = array_intersect_key(
						$delivery_days[ $index ]['time_frames'],
						array_flip( array( $time_frame_id ) )
					);
				}
			}
		}
...
function wc_od_get_subscription_max_delivery_date( $the_subscription ) {
	...
	// Maybe use the 'max_delivery_days' setting if the subscription is not restricted to the billing interval.
	if ( ! wc_od_string_to_bool( WC_OD()->settings()->get_setting( 'subscriptions_limit_to_billing_interval' ) ) ) {
		...
		if ( $max_delivery_timestamp > $timestamp ) {
	...
	if ( wc_od_subscription_needs_delivery_date( $subscription ) ) {
		...
		if ( $delivery_timestamp ) {
			...
			if ( wc_od_subscription_has_delivery_preferences( $subscription ) ) {
				...
				if ( ! wc_od_string_to_bool( $delivery_days[ $wday ]['enabled'] ) ) {
					$shipping_method = wc_od_get_order_shipping_method( $subscription );
					$end_timestamp   = ( wc_od_get_subscription_max_delivery_date( $subscription ) + DAY_IN_SECONDS ); // Non-inclusive.
...
function wc_od_update_subscription_delivery_time_frame( $the_subscription ) {
	...
	if ( $delivery_date ) {
		...
		if ( 1 === count( $time_frames ) ) {
			reset( $time_frames );
			$time_frame = 'time_frame:' . key( $time_frames );
		} elseif ( ! empty( $time_frames ) ) {
...
function wc_od_subscription_delivery_days_field( $field, $key, $args, $value ) {
	...
	<div class="form-row <?php echo esc_attr( implode( ' ', $args['class'] ) ); ?>" id="<?php echo esc_attr( $key ); ?>">
		...
		<table class="<?php echo esc_attr( $class['base'] ); ?> shop_table shop_table_responsive">
			...
			<?php
			foreach ( $delivery_days as $index => $delivery_day ) :
				$enabled = wc_od_string_to_bool( $delivery_day['enabled'] );
				?>
				<tr class="<?php echo esc_attr( $class['row'] ); ?>">
					...
					<td class="<?php echo esc_attr( "{$class['cell']}-time-frame" ); ?>" data-title="<?php echo esc_attr( $columns['time_frame'] ); ?>">
						<?php
						if ( ! $enabled || empty( $delivery_day['time_frames'] ) ) :
							echo '-';
						else :
							$choices  = wc_od_get_time_frames_choices( $delivery_day['time_frames'], 'subscription' );
		...
		<tbody>
			<?php foreach ( $preferred_days as $index => $delivery_day ) : ?>
				<tr>
					...
					<td class="time-frame">
						<?php
						if ( ! wc_od_string_to_bool( $delivery_day['enabled'] ) || empty( $delivery_day['time_frames'] ) ) :
							echo '<span class="na">–</span>';
						else :
							echo wp_kses_post( join( '<br>', array_map( 'wc_od_time_frame_to_string', $delivery_day['time_frames'] ) ) );
						endif;
						?>
					</td>
				</tr>
