...
function wc_od_get_delivery_day( $delivery_day ) {
	if ( ! is_array( $delivery_day ) ) {
		$index         = intval( $delivery_day );
		$delivery_days = WC_OD()->settings()->get_setting( 'delivery_days' );
		$delivery_day  = ( isset( $delivery_days[ $index ] ) ? $delivery_days[ $index ] : array() );
	}
...
function wc_od_get_delivery_day_status( $delivery_day, $args = array(), $context = '' ) {
	...
	$status = wc_od_bool_to_string( $delivery_day['enabled'] );

	if ( 'yes' === $status && $args['shipping_method'] ) {
		$shipping_methods = wc_od_get_shipping_methods_for_delivery_day( $delivery_day );

		$status = wc_od_bool_to_string( empty( $shipping_methods ) || in_array( $args['shipping_method'], $shipping_methods, true ) );
	}
	 ...
	 * @since 1.5.0
	 *
	 * @param string $status       The delivery day status. 'yes' or 'no'.
	 * @param array  $delivery_day The delivery day data.
	 * @param array  $args         The arguments.
	 * @param string $context      The context.
	 */
 ...
 * @since 1.5.0
 *
 * @param array  $delivery_days Optional. The delivery days data to use.
 * @param array  $args          Optional. The arguments.
 * @param string $context       Optional. The context.
 * @return array
 */
...
function wc_od_get_last_shipping_date( $args = array(), $context = '' ) {
	...
	do {
		...
		$days_for_delivery--;
		$wday = ( 0 === (int) $wday ? 6 : $wday - 1 ); // The module operator doesn't work with negative numbers.
...
function wc_od_get_first_delivery_date( $args = array(), $context = '' ) {
	...
	do {
		 ...
		 */
		if ( wc_od_string_to_bool( $delivery_days_status[ $wday ] ) || ( $shipping_timestamp === $timestamp && 0 < $min_delivery_days ) ) {
