...
if ( ! class_exists( 'WC_OD_Checkout' ) ) {
	...
	class WC_OD_Checkout extends WC_OD_Singleton {
		...
		public function get_delivery_date_field_args( $args = array() ) {
			...
			if ( 'auto' === WC_OD()->settings()->get_setting( 'delivery_date_field' ) ) {
				$args['placeholder'] = wc_od_localize_date( $this->get_first_delivery_date() );
			}
		...
		public function get_order_delivery_date() {
			...
			if ( ! $delivery_date && 'auto' === WC_OD()->settings()->get_setting( 'delivery_date_field' ) ) {
		...
		public function get_order_time_frame( $delivery_date ) {
			...
			$time_frame = wc_od_get_time_frame( $delivery_date, $checkout->get_value( 'delivery_time_frame' ) );

			if ( $time_frame ) {
				$time_frame = wc_od_time_frame_to_order( $time_frame );
			}
